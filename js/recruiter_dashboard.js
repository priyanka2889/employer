...
$(document).ready(function(){
	   if(!localStorage.userId)
		...
		//****** on scroll******//
			...
			$("#jobs_list").on("scrollstart",function(){
			...
			$("#saved_jobs_list").on("scrollstart",function(){
			...
			$("#job_applied_list").on("scrollstart",function(){
			...
			$("#appliedCandidate_list").on("scrollstart",function(){
...
function set_jobtype(){
			  ...
			  if(valid) {
						...
						$.ajax({
							...
							{  
								...
								/*navigator.notification.confirm("JOB Posted Successfully",postCallBack, "Confirmation", "Ok");
									$.mobile.loading('show');
									function postCallBack(button){
												$.mobile.loading('hide');
													if(button == 1) {
														$("#jobs_list").empty();
			  ...
			  if(valid) {
						...
						$.ajax({
							...
							{  if(data==1){
								...
								/*navigator.notification.confirm("JOB Saved Successfully",postCallBack, "Confirmation", "Ok");
									$.mobile.loading('show');
									function postCallBack(button){
												$.mobile.loading('hide');
													if(button == 1) {
   ...
   //*****ON CLICK OF Saved job(listing)*****///
		$(document).on("click",'#saved',function(){
			...
			//*****ON CLICK OF Posted job(listing)*****///
		$(document).on("click",'#dashposted',function(){
			 pagetype="posted_jobs";
			$("#jobs_list").empty();
		   searchlist="postedJobs";
			pageinit();
   ...
   //*****ON CLICK OF Saved job(listing)*****///
		$(document).on("click",'#dashsaved',function(){
			$("#saved_jobs_list").empty();
			 pagetype="saved_jobs";
			 searchlist="savedJobs";
			 pageinit();
	...
	//*****ON CLICK OF Saved job(listing)*****///
		$(document).on("click",'#applied',function(){
			...
			searchlist = "job_applied_list";
			 ...
			 job_applied_list();
			 page=2;
			 window.location.hash="#job_applied";
			});	
	   ...
	   $.ajax({
				...
				success: function(data){
					...
					if(item.js_resume_head!="null"||item.js_resume_head!=""){
				     ...
				     $("#resume").html("<a class='ui-btn ui-btn-b' data-ajax='false' data-id='"+url+"|"+item.js_name+"'  id='resumedownload'>Click to Download</a>");
					}
					else{
				...
				$.ajax({
					...
					if(data==1){
						window.location.hash="#dashboard";
							/*navigator.notification.confirm("updated Successfully",postCallBack, "Confirmation", "Ok");
									$.mobile.loading('show');
									function postCallBack(button){
												$.mobile.loading('hide');
													if(button == 1) {
